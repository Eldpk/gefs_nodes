...
def read_and_transform(config, long_fmt='360'):
    ...
    v1, v2, v3 = config['var1'], config['var2'], config['var3']
    data_in = xr.open_dataset(config['filename'])[[v1, v2, v3]]
    ...
    a1 = data_in[v1].values  
    a2 = data_in[v2].values
    a3 = data_in[v3].values


    npy = np.array([
        np.concatenate([t1.flatten(), t2.flatten(), t3.flatten()])
...
def train_som(preferences):
    ...
    preferences['som_config']['input_len'] = 3 * ds.sizes['longitude'] * ds.sizes['latitude']
